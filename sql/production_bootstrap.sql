...
-- README: Idempotent bootstrap for companies-related schema (companies, klienci link, GUS snapshots link, GUS refresh queue). No destructive ops. MariaDB-compatible.
...
SET @has_idx_uq := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'companies' AND INDEX_NAME = 'uq_companies_nip');
SET @has_dup := 1;
SET @sql := IF(@has_companies = 1, 'SELECT IF(EXISTS(SELECT 1 FROM companies WHERE nip IS NOT NULL AND CHAR_LENGTH(TRIM(nip)) > 0 GROUP BY nip HAVING COUNT(*) > 1 LIMIT 1), 1, 0) INTO @has_dup', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @sql := IF(@has_companies = 1 AND @has_idx_uq = 0 AND @has_dup = 0, 'CREATE UNIQUE INDEX uq_companies_nip ON companies(nip)', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
SET @has_idx_uq := (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'companies' AND INDEX_NAME = 'uq_companies_nip');
...
SET @sql := IF(@has_companies = 1 AND @has_idx = 0 AND @has_idx_uq = 0, 'CREATE INDEX idx_companies_nip ON companies(nip)', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
