...
if (!$jobIds) {
    $_SESSION['flash'] = ['type' => 'warning', 'message' => 'Brak rekordów do ponowienia.'];
    header('Location: ' . BASE_URL . '/admin/gus-queue.php');
    exit;
}

$res = requeueByIds($pdo, $jobIds, $limit);
$_SESSION['flash'] = ['type' => 'success', 'message' => 'Requeue: affected=' . $res['affected'] . ', skipped=' . $res['skipped']];

try {
    adminAuditLog(
        $pdo,
        $scope === 'filter' ? 'requeue_filter' : 'requeue_selected',
        $scope,
        $currentUser,
    ...
    error_log('audit requeue bulk failed: ' . $e->getMessage());
}
