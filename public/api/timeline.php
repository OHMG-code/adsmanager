...
if ($campaignId <= 0) {
    ...
    echo json_encode(['success' => false, 'error' => 'Nieprawidłowe campaign_id.'], JSON_UNESCAPED_UNICODE);
    exit;
}

if (!canUserAccessKampania($pdo, $currentUser, $campaignId)) {
    http_response_code(403);
    echo json_encode(['success' => false, 'error' => 'Brak dostępu do kampanii.'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    ...
    error_log('api/timeline: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'error' => 'Nie udało się pobrać timeline.'], JSON_UNESCAPED_UNICODE);
}
exit;
